...
const resolveGatewayPortMock = vi.hoisted(() => vi.fn(() => 31010));
...
describe("cli integration: qr + dashboard token SecretRef", () => {
  ...
  it("uses the same resolved token SecretRef for qr auth validation and dashboard commands", async () => {
    ...
    expect(payload.url).toBe("ws://gateway.local:31010");
    ...
    expect(joined).toContain("Dashboard URL: http://127.0.0.1:31010/");
