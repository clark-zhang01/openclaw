...
const resolveGatewayPort = vi.fn(() => 31010);
...
describe("runDaemonRestart health checks", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["openclaw", "gateway", "--port", "31010"],
      environment: {},
    });
    ...
    waitForGatewayHealthyListener.mockResolvedValue({
      healthy: true,
      portUsage: { port: 31010, status: "busy", listeners: [], hints: [] },
    });
  ...
  it("fails restart when gateway remains unhealthy", async () => {
    ...
    const unhealthy: RestartHealthSnapshot = {
      ...
      staleGatewayPids: [],
      runtime: { status: "stopped" },
      portUsage: { port: 31010, status: "free", listeners: [], hints: [] },
    };
  ...
  it("signals an unmanaged gateway process on stop", async () => {
    ...
    expect(findVerifiedGatewayListenerPidsOnPortSync).toHaveBeenCalledWith(31010);
  ...
  it("fails unmanaged restart when multiple gateway listeners are present", async () => {
    ...
    await expect(runDaemonRestart({ json: true })).rejects.toThrow(
      "multiple gateway processes are listening on port 31010",
    );
  });
