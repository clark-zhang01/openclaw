...
describe("gateway --force helpers", () => {
  ...
  it("returns empty list when lsof finds nothing", () => {
    ...
    expect(listPortListeners(31010)).toEqual([]);
  });
  ...
  it("throws when lsof missing", () => {
    ...
    expect(() => listPortListeners(31010)).toThrow(/lsof not found/);
  });
  ...
  it("kills each listener and returns metadata", () => {
    ...
    const killed = forceFreePort(31010);
  ...
  it("retries until the port is free", async () => {
    ...
    const promise = forceFreePortAndWait(31010, {
      timeoutMs: 500,
      intervalMs: 100,
  ...
  it("falls back to fuser when lsof is permission denied", async () => {
    (execFileSync as unknown as Mock).mockImplementation((cmd: string) => {
      ...
      return "31010/tcp: 4242\n";
    });
    tryListenOnPortMock.mockResolvedValue(undefined);

    const result = await forceFreePortAndWait(31010, { timeoutMs: 500, intervalMs: 100 });
  ...
  it("throws when lsof is unavailable and fuser is missing", async () => {
    ...
    await expect(forceFreePortAndWait(31010, { timeoutMs: 200, intervalMs: 100 })).rejects.toThrow(
      /fuser not found/i,
    );
  });
});
...
describe("gateway --force helpers (Windows netstat path)", () => {
  ...
  it("parses PIDs from netstat output correctly", () => {
    (execFileSync as unknown as Mock).mockReturnValue(makeNetstatOutput(31010, 42, 99));
    expect(listPortListeners(31010)).toEqual<PortProcess[]>([{ pid: 42 }, { pid: 99 }]);
  });
  ...
  it("deduplicates PIDs that appear multiple times", () => {
    (execFileSync as unknown as Mock).mockReturnValue(makeNetstatOutput(31010, 42, 42));
    expect(listPortListeners(31010)).toEqual<PortProcess[]>([{ pid: 42 }]);
  });
  ...
  it("throws a descriptive error when netstat fails", () => {
    ...
    expect(() => listPortListeners(31010)).toThrow(/netstat failed/);
  });
