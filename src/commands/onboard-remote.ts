...
const DEFAULT_GATEWAY_URL = "ws://127.0.0.1:31010";
      ...
      if (mode === "direct") {
        ...
        await prompter.note(
          [
            ...
            `Using: ${suggestedUrl}`,
            "If your gateway is loopback-only, choose SSH tunnel and keep ws://127.0.0.1:31010.",
          ].join("\n"),
          "Direct remote",
        );
      } else {
        ...
        await prompter.note(
          [
            ...
            `ssh -N -L 31010:127.0.0.1:31010 <user>@${host}${target.sshPort ? ` -p ${target.sshPort}` : ""}`,
