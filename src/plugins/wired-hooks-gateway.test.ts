...
describe("gateway hook runner methods", () => {
  it("runGatewayStart invokes registered gateway_start hooks", async () => {
    ...
    await runner.runGatewayStart({ port: 31010 }, { port: 31010 });

    expect(handler).toHaveBeenCalledWith({ port: 31010 }, { port: 31010 });
  });
  ...
  it("runGatewayStop invokes registered gateway_stop hooks", async () => {
    ...
    await runner.runGatewayStop({ reason: "test shutdown" }, { port: 31010 });

    expect(handler).toHaveBeenCalledWith({ reason: "test shutdown" }, { port: 31010 });
  });
