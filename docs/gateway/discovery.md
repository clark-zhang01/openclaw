...
- **Gateway WS (control plane)**: the WebSocket endpoint on `127.0.0.1:31010` by default; can be bound to LAN/tailnet via `gateway.bind`.
- **Direct WS transport**: a LAN/tailnet-facing Gateway WS endpoint (no SSH).
- **SSH transport (fallback)**: remote control by forwarding `127.0.0.1:31010` over SSH.
...
- TXT keys (non-secret):
  ...
  - `gatewayPort=31010` (Gateway WS + HTTP)
