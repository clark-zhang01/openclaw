...
- The Gateway WebSocket binds to **loopback** on your configured port (defaults to 31010).
...
- `openclaw health` and `openclaw status --deep` now reach the remote gateway via `ws://127.0.0.1:31010`.
- `openclaw gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.

Note: replace `31010` with your configured `gateway.port` (or `--port`/`OPENCLAW_GATEWAY_PORT`).
...
When the gateway is loopback-only, keep the URL at `ws://127.0.0.1:31010` and open the SSH tunnel first.
...
- Forward `31010` over SSH (see above), then connect clients to `ws://127.0.0.1:31010`.
