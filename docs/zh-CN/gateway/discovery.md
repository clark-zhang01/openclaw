...
- **Gateway 网关 WS（控制平面）**：默认在 `127.0.0.1:31010` 上的 WebSocket 端点；可通过 `gateway.bind` 绑定到 LAN/tailnet。
- **直连 WS 传输**：面向 LAN/tailnet 的 Gateway 网关 WS 端点（无 SSH）。
- **SSH 传输（回退）**：通过 SSH 转发 `127.0.0.1:31010` 进行远程控制。
...
- TXT 键（非机密）：
  ...
  - `sshPort=22`（或广播的端口）
  - `gatewayPort=31010`（Gateway 网关 WS + HTTP）
