...
如果你通过纯 HTTP 打开仪表板（例如 `http://<lan-ip>:31010/` 或
`http://<tailscale-ip>:31010/`），浏览器运行在**非安全上下文**中，
会阻止 WebCrypto，因此无法生成设备身份。

**修复：**

- 优先通过 [Tailscale Serve](/gateway/tailscale) 使用 HTTPS。
- 或在 Gateway 网关主机上本地打开：`http://127.0.0.1:31010/`。
...
应用连接到端口 `31010` 上的本地 Gateway 网关。如果一直卡住：
