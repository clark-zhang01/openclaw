...
- 将 WebSocket 控制平面绑定到 `127.0.0.1:<port>`（默认 31010）。
...
- 端口优先级：`--port` > `OPENCLAW_GATEWAY_PORT` > `gateway.port` > 默认 `31010`。
...
- 首选 Tailscale/VPN；否则使用 SSH 隧道：
  ...
  ```
- 然后客户端通过隧道连接到 `ws://127.0.0.1:31010`。
...
[Service]
ExecStart=/usr/local/bin/openclaw gateway --port 31010
Restart=always
RestartSec=5
