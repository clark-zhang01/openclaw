...
- Gateway WebSocket 绑定到你配置端口的 **loopback**（默认为 31010）。
...
- `openclaw health` 和 `openclaw status --deep` 现在通过 `ws://127.0.0.1:31010` 访问远程 Gateway 网关。
- `openclaw gateway {status,health,send,agent,call}` 在需要时也可以通过 `--url` 指定转发的 URL。

注意：将 `31010` 替换为你配置的 `gateway.port`（或 `--port`/`OPENCLAW_GATEWAY_PORT`）。
...
当 Gateway 网关仅限 loopback 时，保持 URL 为 `ws://127.0.0.1:31010` 并先打开 SSH 隧道。

## 通过 SSH 的聊天 UI

WebChat 不再使用单独的 HTTP 端口。SwiftUI 聊天 UI 直接连接到 Gateway WebSocket。

- 通过 SSH 转发 `31010`（见上文），然后让客户端连接到 `ws://127.0.0.1:31010`。
