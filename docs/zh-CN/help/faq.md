...
新手引导还可以自动构建 UI 资源。新手引导后，通常在端口 **31010** 上运行 Gateway 网关。
...
- **Tailnet 绑定**：运行 `openclaw gateway --bind tailnet --token "<token>"`，打开 `http://<tailscale-ip>:31010/`，在仪表板设置中粘贴令牌。
- **SSH 隧道**：`ssh -N -L 31010:127.0.0.1:31010 user@host`，然后从 `openclaw dashboard` 打开 `http://127.0.0.1:31010/?token=...`。
...
4. **使用 tailnet 主机名**
   ...
   - Gateway 网关 WS：`ws://your-vps.tailnet-xxxx.ts.net:31010`
...
```
--port > OPENCLAW_GATEWAY_PORT > gateway.port > 默认 31010
```
...
OpenClaw 通过在启动时立即绑定 WebSocket 监听器来强制运行时锁（默认 `ws://127.0.0.1:31010`）。如果绑定因 `EADDRINUSE` 失败，它会抛出 `GatewayLockError` 表示另一个实例已在监听。
...
- 如果是远程，先建隧道：`ssh -N -L 31010:127.0.0.1:31010 user@host` 然后打开 `http://127.0.0.1:31010/?token=...`。
...
1. 使用 WS URL：`ws://<host>:31010`（或 `wss://...` 如果 HTTPS）。
