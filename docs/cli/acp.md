...
# Remote Gateway
openclaw acp --url wss://gateway-host:31010 --token <token>

# Remote Gateway (token from file)
openclaw acp --url wss://gateway-host:31010 --token-file ~/.openclaw/gateway.token
...
openclaw acp client --server-args --url wss://gateway-host:31010 --token-file ~/.openclaw/gateway.token
...
```bash
openclaw config set gateway.remote.url wss://gateway-host:31010
...
```json
{
  "agents": {
    "openclaw": {
      "command": "env OPENCLAW_HIDE_BANNER=1 OPENCLAW_SUPPRESS_NOTES=1 openclaw acp --url ws://127.0.0.1:31010 --token-file ~/.openclaw/gateway.token --session agent:main:main"
    }
  }
}
```
