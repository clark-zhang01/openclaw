...
```bash
openclaw node run --host <gateway-host> --port 31010 --display-name "Build Node"
```
...
```bash
# Terminal A (keep running): forward local 18790 -> gateway 127.0.0.1:31010
ssh -N -L 18790:127.0.0.1:31010 user@gateway-host
...
```bash
openclaw node install --host <gateway-host> --port 31010 --display-name "Build Node"
...
```bash
openclaw node run --host <gateway-host> --port 31010
```
