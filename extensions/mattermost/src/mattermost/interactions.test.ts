...
describe("callback URL registry", () => {
  it("stores and retrieves callback URLs", () => {
    setInteractionCallbackUrl("acct1", "http://localhost:31010/mattermost/interactions/acct1");
    expect(getInteractionCallbackUrl("acct1")).toBe(
      "http://localhost:31010/mattermost/interactions/acct1",
    );
  });
...
describe("resolveInteractionCallbackUrl", () => {
  ...
  it("uses default port 31010 when no config provided", () => {
    const url = resolveInteractionCallbackUrl("myaccount");
    expect(url).toBe("http://localhost:31010/mattermost/interactions/myaccount");
  });
});
...
describe("buildButtonAttachments", () => {
  ...
  it("returns an array with one attachment containing all buttons", () => {
    const result = buildButtonAttachments({
      callbackUrl: "http://localhost:31010/mattermost/interactions/default",
      buttons: [
  ...
  it("sets type to 'button' on every action", () => {
    const result = buildButtonAttachments({
      callbackUrl: "http://localhost:31010/cb",
      buttons: [{ id: "a", name: "A" }],
    });
  ...
  it("passes callback URL to each button integration", () => {
    const url = "http://localhost:31010/mattermost/interactions/default";
    const result = buildButtonAttachments({
      callbackUrl: url,
      buttons: [
        { id: "a", name: "A" },
